// Minimal Cardzone UAT checkout starter
// Single-file Node.js server with:
// - test checkout page
// - mkReq key exchange
// - auto-submit form to Cardzone hosted page (/rreq or /mercReq)
// - callback endpoint to inspect MPIRes
//
// Run:
//   1) Save as cardzone_uat_checkout_server.js
//   2) npm init -y   (optional)
//   3) node cardzone_uat_checkout_server.js
//
// Then open http://localhost:3000
//
// IMPORTANT:
// - This is a starter for UAT testing.
// - Replace all env vars below with the actual values given by Cardzone / your bank.
// - If your UAT callback must be public, use ngrok/cloudflared and set CALLBACK_BASE_URL.
// - The MAC field order follows the Cardzone PDF, but you should confirm exact handling of
//   optional blank fields and line item formatting with Cardzone before production.
...
// In-memory transaction store for UAT demo only.
...
function generateTxnId() {
  ...
  const raw = `${yyyy}${MM}${dd}${hh}${mm}${ss}${rand}`;
...
async function doMkReq({ merchantId, purchaseId, merchantPublicKeyBase64Url, merchantPrivateKeyPem }) {
  ...
  if (ENABLE_MKREQ_MAC) {
    payload.mac = signSha256WithRsaBase64Url(
      mkReqSignString(payload),
...
<head>
  ...
  <style>
    ...
    input,select,textarea{width:100%;padding:12px;border:1px solid #d1d5db;border-radius:10px;box-sizing:border-box}
...
<body>
  <div class="wrap">
    <div class="card">
      ...
      <p class="muted">Small starter page for Hosted Payment Page testing.</p>
      <form method="post" action="/start-payment">
        <div class="grid">
          <div>
            <label>Merchant ID</label>
            <input name="merchantId" value="${escapeHtml(MERCHANT_ID)}" required />
          </div>
          <div>
          ...
          <div>
            <label>Amount</label>
            <input name="amount" value="1.00" required />
          </div>
          <div>
            <label>Order / Purchase ID</label>
            <input name="purchaseId" placeholder="Leave blank to auto-generate" />
          </div>
          <div>
            <label>Order Reference</label>
            <input name="orderRef" placeholder="Optional merchant order reference" />
          </div>
          <div>
            <label>Customer Reference</label>
            <input name="customerRef" placeholder="Optional customer/account reference" />
          </div>
          <div>
            <label>Email</label>
            <input name="email" type="email" value="uat@example.com" />
          </div>
          <div>
            <label>Mobile phone</label>
            <input name="mobilePhone" value="17123456" />
          </div>
          <div>
          ...
          <div>
            ...
            <select name="responseType">
              ...
              <option value="">Default (HTML with callback URL)</option>
            </select>
          </div>
          <div>
        ...
        <div style="margin-top:20px;display:flex;gap:12px;align-items:center">
          <button type="submit">Start UAT Payment</button>
          <span style="color:#6b7280;font-size:14px">This will do <code>mkReq</code> and then auto-post to <code>/mercReq</code>.</span>
        </div>
      </form>
      <div class="note">
        Callback URL currently used by this app: <code>${escapeHtml(CALLBACK_BASE_URL + '/callback')}</code><br />
        If Cardzone must reach your machine from outside, expose this server publicly and update <code>CALLBACK_BASE_URL</code>.
      </div>
    </div>
  </div>
</body>
...
function renderAutoPostPage(action, fields) {
  ...
  return `<!doctype html>
<html>
<head><meta charset="utf-8"><title>Redirecting to Cardzone...</title></head>
